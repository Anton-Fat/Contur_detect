        ...
        : TForm(Owner)
{
 ...
 StringGrid1->ColWidths[0]=StringGrid1->Width*3/4;
 StringGrid1->DefaultRowHeight=StringGrid1->Font->Size
                                  * StringGrid1->Font->PixelsPerInch / 72+5;
 StringGrid2->ColWidths[0]=StringGrid2->Width/4;
 StringGrid2->ColWidths[0]=StringGrid2->Width*3/4;
...
// list<NAbstractOutGate*>::iterator ogl_beg,ogl_end;
 OutGateWatchData* og;
...
/* ogl_beg=GrSender->Model->GetFirstOutGate();
 ogl_end=GrSender->Model->GetLastOutGate();
 while(ogl_beg != ogl_end)
  {
   listI=WatchList.find((*ogl_beg)->GetName());
   if(listI == WatchList.end())
	{
	 og=new OutGateWatchData;
	 og->Legend=(*ogl_beg)->GetName();
	 PossibleWatchList[(*ogl_beg)->GetName()]=og;
	}
...
void __fastcall TSeriesControlForm::Edit3DblClick(TObject *Sender)
{
 ...
 s="Подпись для ";
 s+=StringGrid2->Cells[0][StringGrid2->Row].c_str();
