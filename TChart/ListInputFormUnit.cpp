...
void __fastcall TListInputForm::BitBtn1Click(TObject *Sender)
{
 ...
 if(s.find_first_not_of(' ') == string::npos || s == "")
  if(MustInput)
   {
    ShowError("Ошибка", "Вводимая строка не может быть пустой.");
    return;
   }
 ...
 if(!PresentSelect)
  {
   if(Edit->Text == List->Cells[List->Col][List->Row])
    {
     ShowError("Ошибка", "Данное наименование уже существует. Пожалуйста введите другое.");
     return;
    }
  }
 else
  {
   if(Edit->Text != List->Cells[List->Col][List->Row])
    {
     ShowError("Ошибка", "Необходимо выбрать одну из предлагаемых позиций списка!");
     return;
    }
  }
